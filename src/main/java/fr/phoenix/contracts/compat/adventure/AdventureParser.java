package fr.phoenix.contracts.compat.adventure;


import fr.phoenix.contracts.Contracts;
import fr.phoenix.contracts.compat.adventure.argument.AdventureArgument;
import fr.phoenix.contracts.compat.adventure.argument.AdventureArgumentQueue;
import fr.phoenix.contracts.compat.adventure.argument.EmptyArgumentQueue;
...
import fr.phoenix.contracts.compat.adventure.tag.implementation.*;
import fr.phoenix.contracts.compat.adventure.tag.implementation.decorations.*;
...
public class AdventureParser {
    ...
    public @NotNull String parse(@NotNull final String src) {
        String str = ChatColor.translateAlternateColorCodes(Contracts.plugin.configManager.colorCodeChar, src);
        String cpy = str;
    ...
    public void add(AdventureTag tag) {
        if (tag.backwardsCompatible() && Contracts.plugin.version.isBelowOrEqual(1, 15)) {
            Contracts.plugin.getLogger().warning(String.format("The tag %s is not compatible with your server version.", tag.name()));
            return;
        }
        tags.add(tag);
    }
