...
public abstract class GeneratedInventory extends PluginInventory {
    ...
    private GeneratedInventory prev;

    protected int page, maxPage;
    private boolean hasBeenOpened;

    public GeneratedInventory(PlayerData playerData, EditableInventory editable, GeneratedInventory prev) {
        super(playerData);
        this.prev = prev;
    ...
    public void whenClicked(InventoryClickEvent event) {
        if (event.getClickedInventory() != null && event.getClickedInventory().equals(event.getInventory())) {
            ...
            //General behaviour of certain items
            if (item.getFunction().equals("go-back") && prev != null) {
                prev.open();
                return;
            }
